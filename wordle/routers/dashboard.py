...
def success_pie_chart(user: Annotated[schemas.User, Depends(auth_utils.manager)], db: Annotated[Session, Depends(get_db)]):
...
async def show_dashboard(request: Request, user: Annotated[schemas.User, Depends(auth_utils.manager)], db: Annotated[Session, Depends(get_db)]):
    chart = success_pie_chart(db=db, user=user)
    return templates.TemplateResponse(
        request=request, name="dashboard/dashboard.html", context={"user": user, "success_pie_chart": chart.to_html(full_html=False)}, status_code=200
    )
